...
class RecentAdapter :
    PagingDataAdapter<RecentEntity, RecentAdapter.ViewHolder>(itemDiffCallback) {
    class ViewHolder(private val binding: ItemPostBinding) : RecyclerView.ViewHolder(binding.root) {
        private val shimmerDrawable = ShimmerDrawable()
        fun bind(post: RecentEntity) {
            with(binding) {
    ...
    companion object {
        val itemDiffCallback = object : DiffUtil.ItemCallback<RecentEntity>() {
            override fun areItemsTheSame(
                oldItem: RecentEntity, newItem: RecentEntity
            ): Boolean {
